...
if (botonSuma && botonResta && botonMultiplicacion && botonDivision) {
    botonSuma.addEventListener('click', () => {
        try {
            ...
            document.getElementById('resultado').innerHTML = `Resultado: ${addNumbers(num1, num2)}`;
        }
        catch (error) {
            if (error instanceof Error) {
    ...
    botonResta.addEventListener('click', () => {
        try {
            const [num1, num2] = getNumbers();
            document.getElementById('resultado').innerHTML = `Resultado: ${subtractNumbers(num1, num2)}`;
        }
        catch (error) {
            if (error instanceof Error) {
                document.getElementById('resultado').innerHTML = error.message;
    ...
    botonMultiplicacion.addEventListener('click', () => {
        try {
            const [num1, num2] = getNumbers();
            document.getElementById('resultado').innerHTML = `Resultado: ${multiplyNumbers(num1, num2)}`;
        }
        catch (error) {
            if (error instanceof Error) {
                document.getElementById('resultado').innerHTML = error.message;
    ...
    botonDivision.addEventListener('click', () => {
        try {
            const [num1, num2] = getNumbers();
            document.getElementById('resultado').innerHTML = `Resultado: ${divideNumbers(num1, num2)}`;
        }
        catch (error) {
            if (error instanceof Error) {
                document.getElementById('resultado').innerHTML = error.message;
